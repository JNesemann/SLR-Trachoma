...
master1 <- SLR_import %>%
  ...
  mutate(sumtf=sum(tf_di, na.rm=TRUE),
         ...
         nophoto_yn=case_when(totalid==3 & sumnophotos>=2 ~ 1,
                              ...
                              TRUE ~ NA_real_)) %>%
  select(-complete, -sumtf, -sumti, -sumtfti, -sumungrad, -sumnophotos, -totalid) %>%
  select(id, tf_yn, ti_yn, tfti_yn, ungrad_yn, nophoto_yn, dde, tf_di, ti_di, tfti_di, ungradable, nophotos, everything()) %>% 
  ...
  select(number, camera.instance, id, tf_yn, ti_yn, tfti_yn, ungrad_yn, nophoto_yn, everything()) %>%
...
master1long.slsm.tf <- master1long %>%
  filter(field %in% c("smartphone_1_tf_yn","SLR_1_tf_yn")) %>% mutate(value=as.numeric(value)) %>% select(-field)
...
master1long.smf.tf <- master1long %>%
  filter(field %in% c("smartphone_1_tf_yn","clinic_tf_yn")) %>% mutate(value=as.numeric(value)) %>% select(-field)
...
master1long.slf.tf <- master1long %>%
  filter(field %in% c("SLR_1_tf_yn","clinic_tf_yn")) %>% mutate(value=as.numeric(value)) %>% select(-field)
...
master2wide <- SLR_import %>%
  ...
  mutate(fielddde=paste0(field, dde, sep="_")) %>%
  select(-field, -dde) %>%
  spread(fielddde, value, convert=TRUE)
# Can't get the iccbin to work for some reason...
master2wide %>% ungroup() %>% filter(as.numeric(id)<200) %>% select(tf_di1_, tf_di2_) %>% irr::icc(model = "twoway", type = "agreement", conf.level = 0.95)
master2wide %>% ungroup() %>% filter(as.numeric(id)>=200 & as.numeric(id)<400) %>% select(tf_di1_, tf_di2_) %>% irr::icc(model = "twoway", type = "agreement", conf.level = 0.95)
master2wide %>% ungroup() %>% filter(as.numeric(id)>=400 & as.numeric(id)<600) %>% select(tf_di1_, tf_di2_) %>% irr::icc(model = "twoway", type = "agreement", conf.level = 0.95)
master2wide %>% ungroup() %>% filter(as.numeric(id)>=600 & as.numeric(id)<800) %>% select(tf_di1_, tf_di2_) %>% irr::icc(model = "twoway", type = "agreement", conf.level = 0.95)
master2wide %>% ungroup() %>% filter(as.numeric(id)>=800 & as.numeric(id)<1000) %>% select(tf_di1_, tf_di2_) %>% irr::icc(model = "twoway", type = "agreement", conf.level = 0.95)
master2wide %>% ungroup() %>% filter(as.numeric(id)>=1000) %>% select(tf_di1_, tf_di2_) %>% irr::icc(model = "twoway", type = "agreement", conf.level = 0.95)
...
master1long.slsm.ti <- master1long %>%
  filter(field %in% c("smartphone_1_ti_yn","SLR_1_ti_yn")) %>% mutate(value=as.numeric(value)) %>% select(-field)
...
master1long.smf.ti <- master1long %>%
  filter(field %in% c("smartphone_1_ti_yn","clinic_ti_yn")) %>% mutate(value=as.numeric(value)) %>% select(-field)
...
master1long.slf.ti <- master1long %>%
  filter(field %in% c("SLR_1_ti_yn","clinic_ti_yn")) %>% mutate(value=as.numeric(value)) %>% select(-field)
...
class_metrics <- metric_set(sens, spec) # This sets what you want to calculate, in this case just sens/spec
...
library(purrr)
D <- master1 %>% 
  select(number, age, sex, state_code, age, SLR_1_tf_yn,smartphone_1_tf_yn, clinic_tf_yn, SLR_1_ti_yn,smartphone_1_ti_yn, clinic_ti_yn, examiner) %>%
...
tf.polca2 <- poLCA(tf.form, d2, nclass=2, verbose=FALSE, probs.start=tf.polca2.probs.start.new)
...
# However I later learned that if you repeat the randomLCA command, sometimes class 1 is disease and sometimes clas 2 is disease
# But this randomLCA nice because gives confidence intervals for sens/spec
tiprobs <- outcomeProbs(ti.lca2, boot = TRUE)
tiprobs
# For sensitivity if class 2 is positive disease
tiprobs[[2]]
# For specificity if class 1 is negative disease
